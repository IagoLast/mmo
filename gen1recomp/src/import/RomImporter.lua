...
function RomImporter.new(onComplete, opts)
  ...
  -- The browser reuses the mobiles' save-directory pickup without becoming
  -- "android": web/index.html writes the uploaded cart into the LOVE save
  -- directory (IDBFS) before the engine boots, exactly like the Android SAF
  -- picker writes picked_rom.gb, so findPendingRom below already knows how to
  -- consume it.  It must NOT take the `android` flag, which additionally
  -- drives touch-only UI, the delete-after-import path and love.system
  -- picker/createFile bridges that do not exist under Emscripten.
  local web = Platform.isWeb()
  local saveDirPickup = mobileFileBridge or web
  local CacheFs = require("src.import.CacheFs")
  local self = setmetatable({
    ...
    pickPending = saveDirPickup or nil,
  ...
  if saveDirPickup and needRom then
...
function RomImporter:choose(version)
  ...
  if self.web then
    -- No picker to open: the page owns file selection (a canvas cannot read
    -- the user's disk).  "Import ROM" here means "rescan the save directory",
    -- which is where web/index.html drops the upload.
    local name, data = findPendingRom(self.ready)
    if name then
      self:startData(data, name)
    else
      self.notice = {
        version = self.chooseVersion,
        status = "Choose your ROM on the page, then reload",
        detail = "Use the \"Cambiar ROM\" button above the game, pick a "
          .. "Game Boy .gb/.gbc cart, and the page will reload into it.",
      }
    end
    return
  end
