...
local function buildRomCard(imp, parent, m, version, info, ready, locked)
  -- The browser has no filesystem to drop from and no native picker: the ROM
  -- arrives through the page's own upload control, which has already run by
  -- the time this card is drawn, so point at a reload rather than at a
  -- gesture that cannot work inside the canvas.
  local dropHint = imp.isNX and Strings("Copy the .gb/.gbc via MTP into imports/.")
    or (imp.web and Strings("Use the page's ROM picker, then reload.")
