...
function OverworldState:interact()
  ...
  if npc then
    if npc.remotePlayerId then
      -- Presence NPCs have no ROM dialogue.  Surface the ordinary interaction
      -- event with a distinct kind so the challenge layer can turn A into a
      -- duel request without coupling MMO transport into the overworld.
      interacted(self, fx, fy, "remote_player", npc)
      if Game.mmoBattleFlow then
        Game.mmoBattleFlow:challenge(npc.remotePlayerId)
      end
      return
    elseif npc.pikachuFollower then
