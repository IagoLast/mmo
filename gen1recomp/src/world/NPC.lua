...
function NPC.new(data, mapId, objDef)
  ...
  -- Runtime MMO objects remain ordinary drawable NPCs, with two extra
  -- identity fields consumed by the challenge layer. They are pass-through
  -- because network latency must never make a route temporarily impassable.
  if objDef.mmoPlayerId then
    self.remotePlayerId = tostring(objDef.mmoPlayerId)
    self.remotePlayerName = objDef.mmoPlayerName
...
function NPC:update(map, entities)
  -- MMO presence owns the exact network pose of remote trainers.  They still
  -- use the ordinary NPC pose/draw contract (including voxel pipelines), but
  -- must not run local wandering/interpolation over authoritative coordinates.
  if self.remoteControlled then return end
