...
-- A chip song streams through a queueable Source, and LOVE throws on
-- Source:setLooping for those -- the synth owns looping (allowLoops), so
-- there was never anything to set.  The pcall around the call is not enough
-- on love.js: the C++ love::Exception unwinds straight past Lua into main's
-- handler, which alerts and kills the process.  Ask the Source what it is
-- first, and skip.  A stub Source without getType (headless tests) falls
-- through to the call, as before.
local function setLooping(src, value)
  if not src then return end
  local ok, kind = pcall(src.getType, src)
  if ok and kind == "queue" then return end
  pcall(src.setLooping, src, value)
end
...
function Music.play(data, song, loop, ctx)
  ...
  if loopSrc then
    ...
    setLooping(src, false)
    setLooping(loopSrc, wantLoop)
    applyVolume(loopSrc)
    applyFilter(loopSrc)
  else
    setLooping(src, wantLoop)
  end
  applyVolume(src)
  applyFilter(src)
