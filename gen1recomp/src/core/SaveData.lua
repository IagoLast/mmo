...
function SaveData.defaultOptions()
  -- The MMO launcher opts into a polished, isolated presentation preset.
  -- Keep upstream defaults unchanged for normal gen1recomp launches and
  -- tests, while ensuring source-tree demo mods never alter an MMO save.
  local mmoDefaults = os.getenv("POKEPORT_MMO_DEFAULTS") == "1"
  return {
    ...
    colors = mmoDefaults and "redpp" or "gbc",
    tilt = 0,
    gbcfx = 0,
    ...
    performance = mmoDefaults and "high" or "auto",
    ...
    pipelines = mmoDefaults and { voxel = 4, tiltshift = 1 } or {},
    ...
    mods = mmoDefaults and {
      DRAMATIC_SHAPE = true,
      example_mew_starter = false,
      nuzlocke = false,
    } or {},
    modOptions = mmoDefaults and {
      DRAMATIC_SHAPE = {
        aa = 2,
        battleBack = false,
        battles = true,
