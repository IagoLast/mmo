...
function Sound.startLoop(data, name)
  ...
  if not src then
    ...
    -- a queueable Source throws on setLooping, and on love.js that C++
    -- exception unwinds past Lua and kills the process (see Music.lua):
    -- generated loops are static, but a def is free to hand back anything
    local kindOk, kind = pcall(s.getType, s)
    if not (kindOk and kind == "queue") then pcall(s.setLooping, s, true) end
