...
function Game:load()
  ...
  -- Opt-in only: ordinary/offline games never load a socket or contact a
  -- service. Authentication for the MVP is simply the local save name.
  local mmoAddress = os.getenv("POKEPORT_MMO_ADDR")
  if mmoAddress and mmoAddress ~= "" then
    self:startMMO(mmoAddress)
  end
...
function Game:step(dt)
  ...
  if self.worldClient then
    self.worldClient:update(dt)
    if self.mmoBattleFlow then self.mmoBattleFlow:update(dt) end
  end
...
-- Public seam for tests, launchers and the future web transport.
function Game:startMMO(address, opts)
  opts = opts or {}
  opts.address = address
  if self.worldClient then self.worldClient:close() end
  self.worldClient = require("src.mmo.WorldClient").new(self, opts)
  self.mmoBattleFlow = require("src.mmo.BattleFlow").new(self, self.worldClient,
                                                        opts.battleOptions)
  self.worldClient:connect()
