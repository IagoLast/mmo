...
function love.mousepressed(x, y, button, istouch)
  ...
  -- istouch: the primary touch already arrived via love.touchpressed; SDL's
  -- synthesized mouse twin (Web included -- the browser build forces
  -- POKEPORT_TOUCH on) must not press the overlay a second time.
  if mouseTouch and Game and button == 1 and not istouch then
...
function love.mousemoved(x, y, dx, dy, istouch)
  ...
  if mouseTouch and Game and love.mouse.isDown(1) and not istouch then
...
function love.quit()
  ...
  if Game and Game.worldClient then pcall(function() Game.worldClient:close() end) end
