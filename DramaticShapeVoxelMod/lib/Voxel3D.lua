...
local SHADER = [[
  ...
  // This fragment's place in the sun's view -- and the second varying here
  // that cannot ride GLSL ES's mediump fragment default. The z of it is
  // compared against a depth the sun pass packed into SIXTEEN bits (high
  // byte in red, low in green), and fp16 has about eleven to offer over
  // [0,1]: the comparison then flips on and off in wide stripes running
  // across the sun line, which is what the browser drew over the whole
  // ground. Desktop GL never showed it -- LOVE #defines the precision
  // qualifiers away below GLSL 1.40, so every float there is already
  // single. Same reasoning as vGrid below, and as Water's vBent.
...
#endif
#ifdef PIXEL
  // Qualified for the same reason vSun is, with one extra trap: GLSL ES
  // defaults SAMPLERS to lowp whatever the floats are set to, and eight
  // bits of depth cannot hold a pack that spends sixteen (the low byte
  // would be rounded away and the high one quantised on top). The map is
  // read, not painted, so it wants the precision its contents were
  // written at. Water qualifies its depthTex over the same footnote.
  uniform LOVE_HIGHP_OR_MEDIUMP Image sunMap;
  uniform float sunDark;      // how far into black a shadow goes; 0 = off
  uniform LOVE_HIGHP_OR_MEDIUMP float sunBias;
  uniform LOVE_HIGHP_OR_MEDIUMP vec2 sunTexel;

  // the two-channel pack ShadowMap writes: high byte, then low
  LOVE_HIGHP_OR_MEDIUMP float sunDepth(LOVE_HIGHP_OR_MEDIUMP vec2 uv) {
    LOVE_HIGHP_OR_MEDIUMP vec4 c = Texel(sunMap, uv);
  ...
  float sunlight(LOVE_HIGHP_OR_MEDIUMP vec3 p) {
    ...
    LOVE_HIGHP_OR_MEDIUMP vec2 e = min(p.xy, 1.0 - p.xy);
    float edge = smoothstep(0.0, 0.06, min(e.x, e.y));
    if (edge <= 0.0) return 1.0;
    // the compared depth and the taps that feed it: mediump rounds the
    // bias itself away long before it rounds the pack
    LOVE_HIGHP_OR_MEDIUMP float z = p.z - sunBias;
...
-- in GLES, so this asks for several in order of preference: 24 bits, the
-- same 24 riding a stencil (a pairing some mobile drivers will texture when
-- they refuse the bare format), 32-bit float, and 16 as the floor every
-- GLES3 device can read. Refused all four, beginScene falls straight back
-- to the internal buffer, which is exactly the old behaviour minus the
-- reflections.
local DEPTH_FORMATS = { "depth24", "depth24stencil8", "depth32f", "depth16" }

-- ASKED FIRST, rather than newCanvas'ing each format and catching the
-- refusal. On the desktop the two are the same thing -- LOVE turns the
-- driver's no into a Lua error and pcall eats it -- but under love.js
-- (Emscripten) a love::Exception does NOT arrive as a catchable Lua error:
-- it unwinds through the pcall, prints the driver's complaint to the
-- console, and leaves the caller holding a garbage value where the canvas
-- should be. The next line then indexes it, the whole voxel pipeline
-- errors out, and the engine disables it FOR THE SESSION -- which is why
-- the browser build drew a flat map with no 3D buildings at all. The
-- browser's GL is WebGL 1 (GLES2), where readable `depth24` is simply not
-- a thing, so it hit that path on the very first frame, every time.
--
-- getCanvasFormats answers the same question with a table lookup and
-- throws nothing anywhere. Unknown formats (an engine too old to list the
-- depth ones) fall through to the probe, so nothing regresses off the web.
local function supportedFormats()
  local gfx = love.graphics
  if not (gfx and gfx.getCanvasFormats) then return nil end
  local ok, formats = pcall(gfx.getCanvasFormats, true)
  if not (ok and type(formats) == "table") then return nil end
  -- an engine that lists canvas formats but not depth ones tells us
  -- nothing about depth: treat that as "no answer" rather than "no"
  for _, format in ipairs(DEPTH_FORMATS) do
    if formats[format] ~= nil then return formats end
  end
  return nil
end
...
local function newDepth(w, h)
  ...
  local supported = supportedFormats()
  local c = nil
  for _, format in ipairs(DEPTH_FORMATS) do
    if not supported or supported[format] then
      local ok, made = pcall(love.graphics.newCanvas, w, h,
                             { format = format, readable = true })
      if ok and type(made) == "userdata" then c = made break end
    end
  end
