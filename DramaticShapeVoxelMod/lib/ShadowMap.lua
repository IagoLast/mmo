...
local SHADER = [[
  // The WRITE half of the same precision story the scene shader's vSun
  // tells: this is packed into sixteen bits below, and GLSL ES hands the
  // fragment stage mediump by default -- fp16, which has no low byte to
  // give. Written coarse and read coarse, the compare in the main pass
  // banded the whole ground in stripes along the sun line on every
  // browser build. Off GLSL ES the qualifier is #defined away and this is
  // the float it always was.
  ...
  vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc) {
    ...
    LOVE_HIGHP_OR_MEDIUMP float d = clamp(vDepth, 0.0, 1.0) * 255.0;
...
function ShadowMap.available()
  -- POKEPORT_VOXEL_SUN=0 turns the sun pass off wherever it is set, which
  -- on web is a query parameter away (see web/index.html). Off, VoxelScene
  -- keeps the flat decal shadows -- the same fallback iOS takes below --
  -- so it is a usable mode rather than a broken one, and it is the one
  -- switch that says whether an artefact belongs to this pass at all.
  if os.getenv("POKEPORT_VOXEL_SUN") == "0" then return false end
