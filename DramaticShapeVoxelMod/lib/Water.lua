...
// Qualified, and NOT covered by the `precision highp float` below: that
// statement lifts what comes after it, and a varying declared up here with
// the stage's shared declarations is already mediump by then. Its z is
// compared against a sixteen-bit packed depth (see sunLit), which fp16
// cannot hold -- the scene shader's vSun states the case in full.
...
// the sun's own pass, exactly as the scene shader reads it -- qualifier
// included: samplers default to LOWP on GLSL ES however the floats are
// set, which is the same footnote depthTex carries below, and eight bits
// cannot return a depth that was written in sixteen
