...
-- LuaJIT's 64-bit literals (LL/ULL) are a syntax error to plain Lua 5.1,
-- which is what the web client runs -- and this file is parsed there even
-- though VR can never activate without FFI.  Compile them at runtime
-- instead: exact int64 cdata under LuaJIT, an inert fallback elsewhere.
local compile = loadstring or load
local function int64(literal, fallback)
  local chunk = compile("return " .. literal)
  if chunk then
...
local XR = {
  ...
  INFINITE_DURATION = int64("0x7fffffffffffffffLL", math.huge),
  API_VERSION_1_0 = int64("0x0001000000000000ULL", 2 ^ 48),   -- XR_MAKE_VERSION(1, 0, 0)
