...
-- loadstring where it exists (plain Lua 5.1, the web client's runtime):
-- there `load` only takes a function, so load(source, ...) blows up before
-- the mod gets to run.  LuaJIT and 5.2+ accept either name.
...
local function chunkFor(rel)
  ...
  -- A UTF-8 BOM is whitespace to LuaJIT but a syntax error to plain 5.1.
